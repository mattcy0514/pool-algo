...
class Table:
    ...
    def mirror_table(self, index_ij:np.matrix):
        ...
        # origin transform
        mtable.origin = algo.mirror_transformation(mtable.origin, self.index_rr, index_ij)
        # mball transform
        mtable.mball.pos = algo.mirror_transformation(mtable.mball.pos, self.index_rr, index_ij)
        ...
        for i in range(len(mtable.tballs)):
            ...
            # tballs transform
            mtable.tballs[i].pos = algo.mirror_transformation(mtable.tballs[i].pos, self.index_rr, index_ij)
        # holes transform
        for i in range(len(mtable.holes)):
            mtable.holes[i].pos = algo.mirror_transformation(mtable.holes[i].pos, self.index_rr, index_ij)
            # print(mtable.holes[i].pos)
        for i in range(len(mtable.real_hit_holes_pos)):
            mtable.real_hit_holes_pos[i] = algo.mirror_transformation(mtable.real_hit_holes_pos[i], self.index_rr, index_ij)
